...
export default function PatrimoinePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setAccounts(accts);
      setSettings(s);
    }).catch((err) => console.error("Patrimoine: load error", err))
    ...
    <div style={{ padding: "28px 36px", maxWidth: 1100 }}>
      <div className="mb-8">
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
          <h1 style={{ fontSize: 32, fontWeight: 600, color: "#1D1D1F", letterSpacing: "-0.5px", lineHeight: 1 }}>Patrimoine</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="rounded-apple-lg mb-6" style={{ background: "#F5F5F7", display: "grid", gridTemplateColumns: "1fr 1px 1fr 1px 1fr 1px 1fr 1px 1fr" }}>
            {[
              { label: "Liquidités", value: liquidites, color: "#34C759", sub: "Comptes courants" },
              { label: "Épargne & Bourse", value: epargne, color: "#34C759", sub: "Livrets, PEA" },
              { label: "Immobilier", value: immobilier, color: "#34C759", sub: `SCI ${fek(immoSci)} · 40m² ${fek(immoLille40)} · 19m² ${fek(immoLille19)}` },
              { label: "Crédits restants", value: creditsTotal, color: "#FF3B30", sub: "Capital restant dû total", negate: true },
              { label: "Patrimoine net", value: patrimoineNet, color: patrimoineNet >= 0 ? "#34C759" : "#FF3B30", sub: "Actifs − dettes", bold: true },
            ].map((item, i) => (
              <div key={item.label} style={{ display: "contents" }}>
                ...
                <div style={{ padding: "18px 20px" }}>
                  ...
                  <div style={{
                    fontSize: item.bold ? 22 : 20, fontWeight: item.bold ? 500 : 300,
                    color: item.color, letterSpacing: "-0.6px", lineHeight: 1, marginBottom: 5,
                  }}>
              ...
              <div key={bank} style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                  {accts.map((acct) => {
                    ...
                    const paid = (meta && meta.montant_initial && capitalRestant != null) ? meta.montant_initial - capitalRestant : null;
                    ...
                    return (
                      <div key={acct.id} className="rounded-apple" style={{ background: "#F5F5F7", padding: "16px 18px" }}>
                        ...
                        {/* Balance (read-only) */}
                        <div style={{
                          fontSize: isCredit ? 22 : 20, fontWeight: 300,
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px", marginBottom: 6 }}>
                              {meta.mensualite != null && (
                                <div>
                                  ...
                                  <div style={{ fontSize: 12, fontWeight: 500, color: "#1D1D1F" }}>{fe2(meta.mensualite)}</div>
                                </div>
                              )}
                              {meta.taux != null && (
                                <div>
                                  <div style={{ fontSize: 10, color: "#86868B" }}>Taux</div>
                                  <div style={{ fontSize: 12, fontWeight: 500, color: "#1D1D1F" }}>{meta.taux.toFixed(2)}{"\u202F"}%</div>
                                </div>
                              )}
                              {meta.fin && (
                                <div>
                                  <div style={{ fontSize: 10, color: "#86868B" }}>Fin</div>
                                  <div style={{ fontSize: 12, fontWeight: 500, color: "#1D1D1F" }}>{meta.fin}</div>
                                </div>
                              )}
                              {meta.montant_initial != null && (
                                <div>
                                  <div style={{ fontSize: 10, color: "#86868B" }}>Montant initial</div>
                                  <div style={{ fontSize: 12, fontWeight: 500, color: "#1D1D1F" }}>{fek(meta.montant_initial)}</div>
                                </div>
                              )}
                            </div>
